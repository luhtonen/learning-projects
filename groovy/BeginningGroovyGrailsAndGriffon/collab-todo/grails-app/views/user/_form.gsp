...
<div class="fieldcontain ${hasErrors(bean: userInstance, field: 'categories', 'error')} ">
    ...
    <ul class="one-to-many">
        <g:each in="${userInstance?.categories ?}" var="c">
            <li><g:link controller="category" action="show"
                        id="${c.id}">${c?.encodeAsHTML()}</g:link></li>
        </g:each>
        <li class="add">
            <g:link controller="category" action="create"
                    params="['user.id': userInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'category.label', default: 'Category')])}</g:link>
        </li>
    </ul>
...
<div class="fieldcontain ${hasErrors(bean: userInstance, field: 'todos', 'error')} ">
    ...
    <ul class="one-to-many">
        <g:each in="${userInstance?.todos ?}" var="t">
            <li><g:link controller="todo" action="show"
                        id="${t.id}">${t?.encodeAsHTML()}</g:link></li>
        </g:each>
        <li class="add">
            <g:link controller="todo" action="create"
                    params="['user.id': userInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'todo.label', default: 'Todo')])}</g:link>
        </li>
    </ul>
