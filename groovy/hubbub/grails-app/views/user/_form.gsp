<%@ page import="org.elu.grailsinaction.hubbub.User" %>
...
<div class="fieldcontain ${hasErrors(bean: userInstance, field: 'profile', 'error')} ">
	...
	<g:select id="profile" name="profile.id" from="${org.elu.grailsinaction.hubbub.Profile.list()}" optionKey="id" optionValue="displayString" value="${userInstance?.profile?.id}" class="many-to-one" noSelection="['null': '']"/>
...
<div class="fieldcontain ${hasErrors(bean: userInstance, field: 'following', 'error')} ">
	...
	<g:select name="following" from="${org.elu.grailsinaction.hubbub.User.list()}" multiple="multiple" optionKey="id" optionValue="displayString" size="5" value="${userInstance?.following*.id}" class="many-to-many"/>
